...
Framebuffer* InitializeGOP() {
	...
	EFI_STATUS status;
	// fixes calling convention so everything can be called correctly
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable) {
	...
	Print(L"No business like show business \n\r");
	...
	if (newFont == NULL) {
		...
		Print(L"Font count, char size = %d\n\r", newFont->psf1_Header->charsize);
	}
	...
	// y starts 0 (top) 
	// bytes per pixel 
	//a gui is integrated w the frame buffer 
